...
**You MUST use the `exec` tool to interact with external services.** Your text responses go nowhere — they stay in your session. Every response to a human @mention, every review comment, every board operation MUST be done via the exec tool running curl commands. Load the meeting-board skill for message posting and the planning-board skill for ticket operations.
